<template>
	<view style="background-color: white;">
							...
							<swiper-item>
								<view class="notice-text">
									...
									<p>已更新至 <text style="color: red;margin: 0 6rpx;">{{number}}</text> 个资源 --
										...
										<!-- #endif -->
									<!-- #ifndef H5 -->
									<p>已更新至 <text style="color: red;margin: 0 6rpx;">{{number}}</text> 个资源 -- 复制链接后浏览器打开
									</p>
									<!-- #endif -->
								</view>
							</swiper-item>
		...
		<view v-if="dataList.length != 0" v-for="(item,index) in dataList" :key="index" class="solids-bottom info-list">
			...
			<!-- #ifdef MP-WEIXIN -->
			<text v-if="adnum == 0" @click="copy(item)" class="info-copy">复制链接</text>
			<text v-else @click="openAd(item)" class="info-copy">复制链接</text>
			<!-- #endif -->
			<!-- #ifdef H5 -->
		...
		<!-- #ifdef MP-WEIXIN -->
		<ad-custom v-if="adId && adShow" style="height: 343rpx;margin-top: 20rpx;overflow: hidden;" :unit-id="adId"
			...
			<view style="border-radius: 30rpx;" class="cu-dialog">
				...
				<view class=" bg-white">
					<view style="font-size: 33rpx;color: #6a6a6a;padding: 30rpx 60rpx 60rpx 60rpx;">
						观看 <text style="color: #3C9CFF;margin: 0 10rpx;">视频广告{{adnum}}次后</text> 之后生成链接
					</view>
			...
			getNotice({
				...
				for (var i = 0; i < res.data.length; i++) {
					...
					if (res.data[i].type == 3) {
						...
						that.adUnitId = res.data[i].title
						this.CreateAd()
					}
					//首页小程序分享配置
		...
		methods: {
			...
			toAdUrl(val) {
				...
				if (val.looktype == 1) {
					...
					this.$tab.navigateTo('/pages/adinfo/'+val.path)
				}
				//
			},
			xxfk(val) {
			...
			openAd(val) {
				var that = this;
				console.log(uni.getStorageSync("adnum"))
				if(uni.getStorageSync("adnum") != 3){
					console.log("===1")
					this.urls = val
				...
				if (this.adUnitId != '') {
					if (wx.createRewardedVideoAd) {
						...
						videoAd.onClose((res) => {
							if (res && res.isEnded) {
								this.modalName = null
								uni.setStorageSync("adnum", uni.getStorageSync("adnum")+1)
								that.adnum = that.adnum -1
								//that.copy()
							} else {
								this.modalName = null
								// that.$refs.uToast.show({
